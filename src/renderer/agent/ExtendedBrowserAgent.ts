...
export default class ExtendedBrowserAgent extends BaseBrowserLabelsAgent {
  protected async screenshot(agentContext: AgentContext): Promise<{ imageBase64: string; imageType: 'image/jpeg' | 'image/png' }> {
  ...
  protected async navigate_to(agentContext: AgentContext, url: string): Promise<{ url: string; title?: string }> {
    ...
    if (!isAbsolute) {
      ...
      window.dispatchEvent(new CustomEvent('agent:tab-update', { detail: { tabId: window.__activeTabId, url: window.__activeTabUrl } }));
  ...
  protected async get_all_tabs(agentContext: AgentContext): Promise<Array<{ tabId: number; url: string; title: string }>> {
    return (window.__tabs || []).map(t => ({ ...t }));
  }

  protected async switch_tab(agentContext: AgentContext, tabId: number): Promise<{ tabId: number; url: string; title: string }> {
  ...
  protected async extract_page_content(agentContext: AgentContext, variable_name?: string): Promise<{ title: string; page_url: string; page_content: string; }> {
    let snapshot = await window.__getActivePageSnapshot?.();
    if (!snapshot) {
      // 兜底：仍然使用宿主页面文本（但会包含右侧 UI）
      snapshot = { url: window.__activeTabUrl || location.href, title: document.title, html: document.documentElement.outerHTML, text: document.body?.innerText || '' };
    }
  ...
  private size(): [number, number] {
    return [
      window.innerWidth || document.documentElement.clientWidth || (document.documentElement || document.body).clientWidth,
      window.innerHeight || document.documentElement.clientHeight || (document.documentElement || document.body).clientHeight
    ];
  }

  private sleep(time: number): Promise<void> { return new Promise(r => setTimeout(r, time)); }
}
