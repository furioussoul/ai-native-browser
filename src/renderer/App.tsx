...
export default function App() {
  ...
  const [openApi, setOpenApi] = useState<{ apiKey: string | null; baseUrl: string | null }>({ apiKey: null, baseUrl: null });
  ...
  const startEditTab = useCallback((tabId: number) => {
  ...
  useEffect(() => {
    ...
    const handleDomReady = () => { updateNavigationState(); webview.focus(); };
          ...
          setConversation((prev) => {
            ...
            const findMergeIndex = (): number => {
              switch (payload.type) {
                ...
                case 'thinking':
                case 'tool_running':
                  return updated.findIndex(e => e.role === 'agent' && e.payload.type === payload.type && 'streamId' in e.payload && e.payload.streamId === (payload as any).streamId && e.payload.taskId === payload.taskId && !e.payload.streamDone);
                case 'workflow':
                  return updated.findIndex(e => e.role === 'agent' && e.payload.type === 'workflow' && e.payload.taskId === payload.taskId && !e.payload.streamDone);
                case 'tool_streaming':
                  return updated.findIndex(e => e.role === 'agent' && e.payload.type === 'tool_streaming' && e.payload.toolId === (payload as any).toolId && e.payload.taskId === payload.taskId);
                case 'tool_result':
                case 'agent_result':
                case 'agent_start':
                case 'error':
                ...
                case 'tool_use':
                  for (let i = updated.length - 1; i >= 0; i--) {
                    const e = updated[i];
                    if (e.role === 'agent' && e.payload.type === payload.type && e.payload.taskId === payload.taskId) return i;
                  }
                  return -1;
                default:
                  return -1;
            ...
            if (idx >= 0) {
              ...
              if (target.role === 'agent') {
                ...
                if ((oldPayload.type === 'text' || oldPayload.type === 'thinking' || oldPayload.type === 'tool_running') && (payload.type === oldPayload.type)) {
                  updated[idx] = {
                    ...target,
                    payload: { ...oldPayload, text: (oldPayload as any).text + (payload as any).text, streamDone: (payload as any).streamDone }
                  };
                  return updated;
                }
                if (oldPayload.type === 'workflow' && payload.type === 'workflow') {
                  ...
                  [...oldSteps, ...newSteps].forEach(s => { if (!map.has(s.id)) map.set(s.id, s); });
                  updated[idx] = {
                    ...target,
                    payload: { ...oldPayload, streamDone: payload.streamDone, workflow: { ...oldPayload.workflow, ...payload.workflow, steps: [...map.values()] } }
                  };
                  return updated;
                }
                if (oldPayload.type === 'tool_streaming' && payload.type === 'tool_streaming') {
                  updated[idx] = { ...target, payload: { ...oldPayload, paramsText: (oldPayload as any).paramsText + (payload as any).paramsText } };
                  return updated;
                }
                if (oldPayload.type === 'tool_result' && payload.type === 'tool_result') {
                  updated[idx] = { ...target, payload: { ...oldPayload, toolResult: { ...oldPayload.toolResult, ...payload.toolResult }, params: { ...oldPayload.params, ...payload.params } } };
                  return updated;
                }
                if (oldPayload.type === 'agent_result' && payload.type === 'agent_result') {
                  updated[idx] = { ...target, payload: { ...oldPayload, result: payload.result ?? oldPayload.result, error: payload.error ?? oldPayload.error } };
                  return updated;
                }
                if (oldPayload.type === 'agent_start' && payload.type === 'agent_start') {
                  updated[idx] = { ...target, payload: { ...oldPayload, agentNode: { ...oldPayload.agentNode, ...payload.agentNode } } };
                  return updated;
                }
                ...
                if (oldPayload.type === 'finish' && payload.type === 'finish') {
                  updated[idx] = { ...target, payload: { ...oldPayload, finishReason: payload.finishReason, usage: payload.usage } };
                  return updated;
                }
                if (oldPayload.type === 'tool_use' && payload.type === 'tool_use') {
                  updated[idx] = { ...target, payload: { ...oldPayload, params: { ...oldPayload.params, ...payload.params } } };
                  return updated;
                }
              }
            }
    ...
    <div className="flex h-full flex-col bg-surface">
      <header className="flex items-center justify-between gap-3 border-b border-white/10 bg-gradient-to-r from-[#1a1c2c] to-[#13141f] px-3 py-2">
        ...
        <div className="flex items-center gap-2">
          <button type="button" title="新建标签页" onClick={handleNewWindow} className={actionButtonClass}>＋</button>
          <button type="button" title="开发者工具" onClick={handleToggleDevtools} className={actionButtonClass}>{'{ }'}</button>
        </div>
      </header>
      ...
      <main className="flex flex-1 min-h-0 bg-transparent">
        <section className="flex-1 min-w-0 flex flex-col p-3 pr-0">
            ...
            {tabs.map(t => (
              ...
              >
                {editingTabId === t.tabId ? (
                  <form
                    onSubmit={(e) => { e.preventDefault(); commitEditTab(); }}
                  ...
                  >
                    <input
                      ...
                      onKeyDown={e => { if (e.key === 'Escape') { e.preventDefault(); cancelEditTab(); } }}
                      spellCheck={false}
                  ...
                  <span className="max-w-[160px] truncate" title={t.url}>{t.title || t.url}</span>
                )}
                {t.loading && <span className="inline-block h-2 w-2 animate-pulse rounded-full bg-blue-400" />}
                {tabs.length > 1 && (
                  <button
                    type="button"
          ...
          <div className="flex-1 relative">
            {tabs.map(t => (
              <webview
                key={t.tabId}
                ref={el => { webviewRefs.current[t.tabId] = el as unknown as WebviewTag | null; }}
        ...
        <aside className={agentPanelClasses} aria-label="AI 交互区">
          <header
            className={`flex w-full items-center gap-3 border-b border-white/10 px-4 py-3 ${isAgentCollapsed ? 'justify-center' : 'justify-between'
              }`}
          >
            {!isAgentCollapsed && (
